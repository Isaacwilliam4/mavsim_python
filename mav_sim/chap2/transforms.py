...
def rot_b_to_v(psi: float, theta: float, phi: float) -> RotMat:
    ...
    """
   # Calculate rotaiton matrix
    return rot_v_to_b(psi, theta, phi).T
...
def trans_i_to_b(pose: Pose, p_i: Points) -> Points:
    ...
    p_b = p_i.copy()
    p_b = trans_i_to_v(pose, p_b)

    m = rot_v_to_b(psi=pose.psi, theta=pose.theta, phi=pose.phi)
...
def trans_b_to_i(pose: Pose, p_b: Points) -> Points:
    ...
    p_i = p_b.copy()
    m = rot_v_to_b(psi=pose.psi, theta=pose.theta, phi=pose.phi).T
    p_i = m@p_i
    p_i = trans_v_to_i(pose, p_i)

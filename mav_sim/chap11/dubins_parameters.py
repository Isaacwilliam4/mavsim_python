...
def compute_parameters(points: DubinsPoints) -> DubinsParamsStruct:
    ...
    dubin = calculate_rsr(points=points)

    rsl_dubin = calculate_rsl(points=points)
    if rsl_dubin.L < dubin.L:
        dubin = rsl_dubin
    lsr_dubin = calculate_lsr(points=points)
    if lsr_dubin.L < dubin.L:
        dubin = lsr_dubin
    lsl_dubin = calculate_lsl(points=points)
    if lsl_dubin.L < dubin.L:
        dubin = lsl_dubin

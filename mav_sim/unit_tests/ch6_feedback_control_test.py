...
def pd_with_rate_tests(tests: list[PDControlWithRateTest]) -> bool:
    ...
    for test in tests:
        ...
        out: list[float] = []
        good_types = True # Indicates whether an incorrect type was found
        for (y_ref, y, ydot) in zip(test.y_ref, test.y, test.ydot):
            # Generate inputs and output
            ctrl_output = ctrl.update(y_ref=y_ref, y=y, ydot=ydot)
            out.append( ctrl_output )

            # Check the type
            if not isinstance(ctrl_output, float):
                good_types = False # type: ignore[unreachable]

        # Test the results
        if not np.allclose(out, test.outputs):
            success = False
            print("\n\nFailed test!")
            print("test:\n", test, "\nActual_output:\n ", out)
            break
        if not good_types:
            success = False
            print("\n\nFailed test due to type mismatch!")
            print("test:\n", test, "\nActual_output:\n ", out)
            break
...
def pi_control_tests(tests: list[PIControlTest]) -> bool:
    ...
    for test in tests:
        ...
        for (y_ref, y) in zip(test.y_ref, test.y):
            # Generate inputs and output
            ctrl_output = ctrl.update(y_ref=y_ref, y=y)
            out.append( ctrl_output )
...
def tf_control_tests(tests: list[TFControlTest]) -> bool:
...
def autopilot_tests(tests: list[AutopilotTest], threshold: float = 1e-4) -> bool:
    ...
    for test in tests:
        ...
        for (cmd, state, delta, commanded_state) in zip(test.cmd, test.state, test.delta, test.commanded_state):
            ...
            if not isinstance(delta_out.elevator, float) or  \
               not isinstance(delta_out.aileron, float) or \
                not isinstance(delta_out.rudder, float) or \
                not isinstance(delta_out.throttle, float) or \
                not isinstance(command_state_out.north, float) or \
                not isinstance(command_state_out.east, float) or \
                not isinstance(command_state_out.altitude, float) or \
                not isinstance(command_state_out.phi, float) or \
                not isinstance(command_state_out.theta, float) or \
                not isinstance(command_state_out.psi, float) or \
                not isinstance(command_state_out.Va, float) or \
                not isinstance(command_state_out.alpha, float) or \
                not isinstance(command_state_out.beta, float) or \
                not isinstance(command_state_out.p, float) or \
                not isinstance(command_state_out.q, float) or \
                not isinstance(command_state_out.r, float) or \
                not isinstance(command_state_out.Vg, float) or \
                not isinstance(command_state_out.gamma, float) or \
                not isinstance(command_state_out.chi, float) or \
                not isinstance(command_state_out.wn, float) or \
                not isinstance(command_state_out.we, float) or \
                not isinstance(command_state_out.bx, float) or \
                not isinstance(command_state_out.by, float) or \
                not isinstance(command_state_out.bz, float):
                good_types = False # type: ignore[unreachable]


        # Test the results
        if diff >= threshold or not good_types:

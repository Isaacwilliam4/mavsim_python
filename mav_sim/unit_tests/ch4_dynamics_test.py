"""ch4_derivatives_tests.py: Implements some basic tests for the chapter 4 material."""
...
#############  auto test run ######################
def gravitational_force_tests(tests: list[GravitationalForceTest]) -> bool:
    ...
    for test in tests:
        ...
        if out.shape != test.out.shape or not np.allclose(out, test.out):
            print("\n\nFailed test!")
            print("test:\n", test, "\nactual result:\n", out)
            success = False
            break
...
def lateral_aerodynamics_tests(tests: list[LateralDynamicsTest]) -> bool:
    ...
    for test in tests:
        ...
        if  not np.allclose(f_lat, test.out_f_lat) or \
            not np.allclose(torque_lat, test.out_torque_lat) or \
            f_lat.shape != test.out_f_lat.shape or \
            torque_lat.shape != test.out_torque_lat.shape:
            print("\n\nFailed test!")
            print("test:\n", test, "\nactual result: f_lat = \n", str(f_lat) )
            print("torque_lat = \n", str(torque_lat) )
            success = False
...
def longitudinal_aerodynamics_tests(tests: list[LongitudinalDynamicsTest]) -> bool:
    ...
    for test in tests:
        ...
        if  not np.allclose(f_lon, test.out_f_lon) or \
            not np.allclose(torque_lon, test.out_torque_lon) or \
            f_lon.shape != test.out_f_lon.shape or \
            torque_lon.shape != test.out_torque_lon.shape:
...
def forces_moments_tests(tests: list[ForcesMomentsTest]) -> bool:
...
def motor_thrust_torque_tests(tests: list[MotorThrustTorqueTest], threshold: float = 1e-4) -> bool:
    ...
    for test in tests:
        ...
        if diff > threshold or not isinstance(T_p, float) or not isinstance(Q_p, float):
            print("\n\nFailed test!")
            print("test:\n", test, "\nactual T_p: ", T_p, "\nactual Q_p: ", Q_p)
            success = False
            break
...
def update_velocity_data_tests(tests: list[UpdateVelocityTest], threshold: float = 1e-4) -> bool:
    ...
    for test in tests:
        # Generate the output
        Va, alpha, beta, wind_inertial_frame = update_velocity_data(state=test.state, wind=test.wind )

        # Compare the output with the result
        diff = np.linalg.norm(Va-test.Va) + np.linalg.norm(alpha-test.alpha) + \
               np.linalg.norm(beta-test.beta)

        if wind_inertial_frame.shape != test.wind_inertial_frame.shape or not isinstance(Va, float) or not isinstance(alpha, float) or not isinstance(beta, float):
            print("\n\nFailed test - Incorrect data shapes or types!")
            print("test:\n", test, "\nactual Va: ", Va, "\nactual alpha: ", alpha)
            print("actual beta: ", beta, "\nactual wind_inertial_frame:\n", wind_inertial_frame)
            success = False
            break
        if diff > threshold or not np.allclose(wind_inertial_frame, test.wind_inertial_frame):
            print("\n\nFailed test!")
            print("test:\n", test, "\nactual Va: ", Va, "\nactual alpha: ", alpha)
            print("actual beta: ", beta, "\nactual wind_inertial_frame:\n", wind_inertial_frame)
            success = False
...
def wind_simulation_tests(tests: list[WindSimulationTest]) -> bool:
    ...
    for test in tests:
        ...
        # Generate the input / output data for the result
        output_count = 0
        for in_out in zip(test.updates, test.outputs):
            ...
            if not np.allclose(output, y) or not isinstance(y, float):
                success = False
                print("\n\nFailed test!")
